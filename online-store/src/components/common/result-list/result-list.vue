<template>
  <div class="result-list">
    <ul>
      <li v-for="(item,index) in resultData" class="result-item" :class="{'hover':hoverIndex===index}"
          @mouseover="mouseover(index)" @contextmenu.prevent="contextmenuFn(item.objectId)">
        ...
        <div class="item-intro">
          <p><span class="item-title" @click="itemClick(index)">{{ item.name }}来源于什么什么什么</span><i class="iconfont">&#xe682;</i><span
            class="item-time float-right">{{item.updateTime}}</span></p>

          <i class="iconfont icon-tag">&#xe711;</i>
          <span v-if="!item.tags.length">暂无</span>
          <Tag v-if="item.tags.length" class="tag-btn" v-for="(tagItem,index) in item.tags" :key="index">
            {{tagItem.tagName}}
          </Tag>
        </div>
      </li>
